...
@implementation NSString (PasswordLevel)
#pragma mark - 密码强度判断
/// 密码强度等级
- (ZHHPasswordLevel)zhh_passwordLevel{
    NSInteger level = [self zhh_checkPasswordStrength];
    switch (level) {
        case 0:
        case 1:
        case 2:
        case 3:
            return ZHHPasswordLevelEasy;
        case 4:
        case 5:
        case 6:
            return ZHHPasswordLevelMidium;
        case 7:
        case 8:
        case 9:
            return ZHHPasswordLevelStrong;
        case 10:
        case 11:
        case 12:
            return ZHHPasswordLevelVeryStrong;
        default:
            return ZHHPasswordLevelExtremelyStrong;
    }
}
/// 检查密码强度
- (NSInteger)zhh_checkPasswordStrength{
    if ([self zhh_isNull] || [self zhh_isCharEqual]) {
        return 0;
    }
    // 数字类型
    static int NUM = 1;
    ...
    if (([self zhh_isNumeric] && [@"01234567890" containsString:self]) ||
    ...
    if (len % 2 == 0) { // aaabbb
        ...
        if ([part1 zhh_isCharEqual] && [part2 zhh_isCharEqual]) {
            level--;
        }
    }
    if (len % 3 == 0) { // ababab
    ...
    if ([self zhh_isNumeric] && len >= 6) { // 19881010 or 881010
        NSInteger year = 0;
        if (len == 8 || len == 6) {
