...
/**
 * @brief 设置视图的边框颜色。
 * @param zhh_borderColor 边框颜色。
 */
- (void)setZhh_borderColor:(UIColor * _Nullable)zhh_borderColor {
 ...
 * @return 当前边框颜色。
 */
- (UIColor * _Nullable)zhh_borderColor {
...
#pragma mark - Corner Radius Properties
/**
 * @brief 设置视图的圆角半径。
 * @param zhh_cornerRadius 圆角半径。
 */
- (void)setZhh_cornerRadius:(CGFloat)zhh_cornerRadius {
    self.layer.cornerRadius = zhh_cornerRadius;
    self.layer.masksToBounds = (zhh_cornerRadius > 0);

    // 性能优化：只有在需要时启用栅格化以提高复杂视图的滚动性能
    if (zhh_cornerRadius > 0) {
