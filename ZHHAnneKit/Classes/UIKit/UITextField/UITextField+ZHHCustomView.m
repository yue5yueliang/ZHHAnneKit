...
@implementation UITextField (ZHHCustomView)
+ (void)load {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        method_exchangeImplementations(class_getInstanceMethod(self.class, NSSelectorFromString(@"layoutSubviews")),
                                       class_getInstanceMethod(self.class, @selector(zhh_custom_layoutSubviews)));
    });
}

- (void)zhh_custom_layoutSubviews{
    [self zhh_custom_layoutSubviews];
    ...
    ZHHTextFieldLeftMaker *maker = objc_getAssociatedObject(self, &leftViewMakerKey);
    if (maker) {
...
- (UIView *)zhh_leftView:(void(^)(ZHHTextFieldLeftMaker *))withBlock{
    ZHHTextFieldLeftMaker * maker = [[ZHHTextFieldLeftMaker alloc] init];
...
/// 配置左边控件相关尺寸信息
- (void)setupLeftViewFrameWithMaker:(ZHHTextFieldLeftMaker *)maker leftView:(UIView *)view{
    ...
    if (maker.text && maker.imageName) {
        CGFloat tw = [self zhh_calculateRectWithText:maker.text
        ...
        if (maker.style == ZHHTextAndImageStyleNormal) {
...
/// 获取文本尺寸
- (CGSize)zhh_calculateRectWithText:(NSString *)string
                               size:(CGSize)size
                               font:(UIFont *)font
...
@implementation ZHHTextFieldLeftMaker
